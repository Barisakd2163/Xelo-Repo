...
class OxAx : MainAPI() {
    override var mainUrl              = "https://ythls.XeloRepo.org/oxax/cs3"
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val channels = app.get(request.data).parsedSafe<XeloAPI>()!!.channels.map { channel ->
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        return app.get("${mainUrl}/search/${query}").parsedSafe<XeloAPI>()!!.channels.map { channel ->
            newLiveSearchResponse(channel.detail.title, "http://oxax.tv/${channel.slug}.html", TvType.Live) {
                this.posterUrl = channel.detail.img
                this.lang = "RU"
            }
